...
export function ClueItem({ word, isActive, isSolved, onClick }: ClueItemProps) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={`flex w-full cursor-pointer items-start gap-2.5 rounded-xl p-2.5 px-3 text-left transition-all hover:bg-surface-soft ${
        isActive
          ? 'relative border border-brand-soft bg-brand-softer before:absolute before:bottom-2 before:left-0 before:top-2 before:w-[3px] before:rounded-r before:bg-brand'
          : ''
      } ${isSolved ? 'opacity-65' : ''}`}
    >
      <span
