...
export function ActiveClueCard({
  word,
  onPrev,
  onNext,
  compact = false,
  ...
  return (
    <motion.div
      ...
      className={`flex w-full shrink-0 items-center gap-3 rounded-2xl border border-brand-soft bg-white shadow-sm ${
        compact ? 'p-2.5 px-3' : 'gap-4 p-4 px-5'
      } ${className}`}
    >
      {word ? (
        <>
